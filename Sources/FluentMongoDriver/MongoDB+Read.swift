...
extension FluentMongoDatabase
{
  ...
  ) async throws
  {
    do
    {
      ...
      let find = raw[query.schema].find(condition)
      ...
      for field in query.fields
      {
        ...
        try projection.include(FieldPath(stringLiteral: field.makeMongoPath()))
      }
      find.command.projection = projection.document

      find.command.sort = try query.makeMongoDBSort()?.stage
